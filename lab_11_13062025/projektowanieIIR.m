...
stem(xs,"r");title("x");
subplot(3,1,2);
stem(real(h1),"b");title("h");
subplot(3,1,3);
stem(real(y),"g");title("y");
...
figure(11);
plot(Fa/(2*pi), 20*log10(abs(Ha)), 'b'); hold on;
plot(F1, 20*log10(abs(H1)), 'r'); hold off;
xlim([0, fs/2]);
xlabel('Frequency (Hz)');
ylabel('Magnitude (dB)');
legend('Ha', 'H1');
title('Transmitancja filtrów H1 i Ha');
grid on;
