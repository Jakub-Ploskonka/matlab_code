...
if (mod(N,2) == 0)
    ...
    F = -2*(fs+Df/2):df:2*(fs-Df/2);
end

X = calcDFT(x, 0:N-1);
S = calcDFT(x,-2*N:2*N-1);
f3 = -2*N*Df:Df:(2*N)*Df-Df;

figure(2);
subplot(2,1,1);
stem(f3,fftshift(abs(S)),".", MarkerSize=10);hold on;
subplot(2,1,2);
...
for n=1:length(F)
    S_od(n) =calka(s.*exp(-j*2*pi*F(n)*t),dt);
end
S_od = S_od*fs;
