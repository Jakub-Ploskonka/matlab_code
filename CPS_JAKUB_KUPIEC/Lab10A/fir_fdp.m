...
% okno hamminga Rr=-53
a = 3.3; %współczynnik z wykładu 

Rmin = a*(fs/(fr-fp));
R = ceil(Rmin); %Dobranie rzędu filtru
...
H = xpi(f/(2*f0/fs)).*exp(-j*2*pi*f*R/2); %transmitancja filtra
...
figure();
stem(F1,abs(H1),"g.");hold on; grid on;
xlim([0,fs/2]);
stem(F,abs(H),"r."); 
